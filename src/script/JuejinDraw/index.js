!(({ api, console, utils }) => {
  ...
  !(async () => {
    ...
    const lottery = async () => {
      const result = await api.tenDrawLottery()
      const { LotteryBases, draw_lucky_value, total_lucky_value } = result
      // state.sumPoint -= state.pointCost
      // state.sumPoint += getSupplyPoint(result)
      // state.luckyValue += draw_lucky_value
      // state.counter += 10
      // state.prize[result.lottery_name] = (state.prize[result.lottery_name] || 0) + 1
      console.log(LotteryBases.map((item) => item.lottery_name).join('\n'))
    }
  ...
  (() => {
    ...
    const api = (() => {
      return {
        ...
        async tenDrawLottery() {
          return this.post('/lottery/ten_draw')
        },
        async checkIn() {
